...
class ConformerGenerator(object):
    ...
    def get_mol_from_xyz(
        ...
        if use_xyz2mol_vdw in ["force", "fallback"]:
            ...
            if use_xyz2mol_vdw == "force" or not xyz_mol:
                conformer, atom_symbols, comment = (
                    ConformerGenerator.load_conformer_from_xyz_file(xyz_file)
                )
                rwMol = Chem.RWMol()
                ...
                for i in range(num_atoms):
                    ...
                    for j in range(i + 1, num_atoms):
                        ...
                        Rcov_j = (
                            pt.GetRcovalent(a_j.GetAtomicNum()) * vdw_covalent_factor
                        )
    ...
    def EmbedMolecule_with_balloon(cls, mol, number_of_conformers_to_generate=1):
        ...
        number_of_conformers_missing = (
            number_of_conformers_to_generate - mol.GetNumConformers()
        )
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                command = [
                    balloon_executable,
                    "--nconfs",
                    str(number_of_conformers_missing),
                    "--noGA",
                    "--njobs",
                    "4",
                    "--randomSeed",
                    ...
                    "--output-format",
                    "sdf",
                    temporary_in_MOL_file,
                    temporary_out_SDF_file,
                ]
                _ = spr.run(command, capture_output=True)

                with Chem.SDMolSupplier(
                    temporary_out_SDF_file, removeHs=False, sanitize=False
                ) as suppl:
                    for conf_mol in suppl:
                        conf = Chem.Conformer(conf_mol.GetConformer())
                        conf.SetDoubleProp(
                            "energy", conf_mol.GetDoubleProp("energy")
                        )  # kcal/mol
    ...
    def get_embedded_mol(cls, smiles, xyz_file=None, charge=0):
        ...
        if xyz_file is None:
            ...
            if retVal < 0:
                try:
                    retVal = rdDistGeom.EmbedMolecule(
                        mol, randomSeed=0xF00D, useRandomCoords=True
                    )
                except Exception:
                    pass
            ...
            if retVal == 0 and mol.GetNumAtoms() > 2:
                ...
                if rdForceFieldHelpers.MMFFHasAllMoleculeParams(mol):
                    structure_was_optimized_successfully = (
                        rdForceFieldHelpers.MMFFOptimizeMolecule(mol) != 0
                    )
    ...
    def save_conformer_to_xyz_file(conformer, filepath_xyz, comment=None):
        ...
        with open(filepath_xyz, "w") as xyzf:
            ...
            for i_atom in range(n_atoms):
                xyzf.write(
                    "{:s}  {:.16f}  {:.16f}  {:.16f}\n".format(
                        mol.GetAtomWithIdx(i_atom).GetSymbol(),
                        atom_positions[i_atom][0],
                        atom_positions[i_atom][1],
                        atom_positions[i_atom][2],
                    )
                )
        ...
        with open(filepath_xyz, "r") as xyzf:
            ...
            for i_atom in range(n_atoms):
                ...
                conformer.SetAtomPosition(
                    i_atom, Point3D(float(line[1]), float(line[2]), float(line[3]))
                )

        return conformer, atom_symbols, comment

    @staticmethod
    def calculate_best_rms(
        probe_molecule,
        reference_molecule,
        probe_molecule_conformer_index=0,
        reference_molecule_conformer_index=0,
        ...
        return rdMolAlign.GetBestRMS(
            probe_mol,
            reference_mol,
            probe_molecule_conformer_index,
            reference_molecule_conformer_index,
        )

    @staticmethod
    def guess_needed_ressources(mol):

        number_of_heavy_atoms = sum(
            [1 if atom.GetSymbol() != "H" else 0 for atom in mol.GetAtoms()]
        )
    ...
    def __init__(
        ...
        self._continue_calculation = continue_calculation and os.path.exists(
            self.dir_job
        )
        self.step_names = []
        self.step_folders = []
    ...
    def __exit__(self, exception_type, exception_value, exception_traceback):
        ...
        if exception_type is None:
            ...
            self._cleanup()
            print(
                f'end: {self.name}, time_spent: {"{:.2f}".format(time.monotonic() - self._start_time)} s'
            )

    def setup_initial_structure(
        self, smiles_or_molecule, xyz_file=None, charge=None, title=None
    ):
        ...
        try:
            ...
            if isinstance(smiles_or_molecule, str):
                ...
                if smiles_or_molecule:
                    ...
                    self.mol = ConformerGenerator.get_embedded_mol(
                        self.smiles, xyz_file, charge
                    )
            else:
            ...
            self.mol = None
            raise ValueError(
                f"The structure could not be setup using the given values, {smiles_or_molecule=}, {xyz_file=}, {charge=}"
            )
        ...
        if self.charge != charge:
            raise ValueError(
                f"The charge of following molecule does not agree with the smiles: {self.name}"
            )
        ...
        auto_max_RAM_per_core_in_MB, auto_n_cores = (
            ConformerGenerator.guess_needed_ressources(self.mol)
        )
        ...
        should_skip_step_execution = self._add_step(
            "setup_initial_structure", title=title
        )
        ...
        if not os.path.isdir(self.dir_job):
            ...
            if not self._continue_calculation:
                ...
                if step_dirs:
                    ...
                    for dir_ in all_step_dirs:
                        if dir_ not in step_dirs:
                            if (
                                int(dir_.split("_")[0])
                                > self.last_completed_step_number
                            ):
                                shutil.rmtree(os.path.join(self.dir_job, dir_))
                                time.sleep(1)
                            else:
                                raise RuntimeError(
                                    "Unexpected step was going to be deleted while continuing a previous calculation."
                                )
                    ...
                    if os.path.exists(last_step_out_dir):
                        ...
                        last_step_calculate_dir = os.path.join(
                            self.dir_job, last_step_dir, "calculate"
                        )

                        if os.path.exists(last_step_calculate_dir):
                            last_step_calculate_folders = sorted(
                                os.listdir(last_step_calculate_dir)
                            )
                            if last_step_calculate_folders:
                                last_step_calculate_folder = (
                                    last_step_calculate_folders[-1]
                                )
                                shutil.rmtree(
                                    os.path.join(
                                        self.dir_job,
                ...
                if not last_step_dir:
                    raise RuntimeError(
                        "Could not find last step while continuing a previous calculation."
                    )
    ...
    def _add_step(self, step_name, title=None):
        ...
        if self._continue_calculation:
            should_skip_step_execution = (
                self.current_step_number < self.last_completed_step_number
            )
        ...
        self._log_results(
            title=title, should_skip_step_execution=should_skip_step_execution
        )

        if should_skip_step_execution:
            self.mol.RemoveAllConformers()
            mol_atom_symbols = [atom.GetSymbol() for atom in self.mol.GetAtoms()]
            for conformer_id, filepath_xyz in enumerate(
                sorted(os.listdir(os.path.join(self._dir_step, "out")))
            ):
                conformer, atom_symbols, comment = (
                    ConformerGenerator.load_conformer_from_xyz_file(
                        os.path.join(self._dir_step, "out", filepath_xyz)
                    )
                )

                if mol_atom_symbols != atom_symbols:
                    raise ValueError(
                        f"Atom order from the molecule does not agree with the atom order of the conformer."
                    )
    ...
    def calculate_rdkit(
        self,
        number_of_conformers_to_generate=None,
        rms_threshold=0.5,
        rms_only_heavy_atoms=True,
        fallback_on_balloon_for_conformer_generation=True,
    ):
        ...
        if number_of_conformers_to_generate is None:
            ...
            try:
                ...
                pass
            try:
                number_of_rotable_bonds = Chem.rdMolDescriptors.CalcNumRotatableBonds(
                    temp
                )
            except Exception:
                number_of_rotable_bonds = (
                    8  # in case errors happen, use middle heuristic
                )
        ...
        print(
            f"- settings used: [number_of_conformers_generated: {number_of_conformers_to_generate}, rms_threshold: {rms_threshold}, rms_only_heavy_atoms: {rms_only_heavy_atoms}]"
        )
        ...
        if not should_skip_step_execution:
            ...
            try:
                _ = rdDistGeom.EmbedMultipleConfs(
                    self.mol,
                    numConfs=number_of_conformers_to_generate,
                    randomSeed=0xF00D,
                )
            ...
            if (
                self.mol.GetNumConformers() != number_of_conformers_to_generate
                and fallback_on_balloon_for_conformer_generation
            ):
                _ = self.EmbedMolecule_with_balloon(
                    self.mol, number_of_conformers_to_generate
                )
                print(
                    f"- rdkit conformer generation failed, balloon (MMF94) was used instead"
                )

                energies = np.array(
                    [conf.GetDoubleProp("energy") for conf in self.mol.GetConformers()]
                )
            ...
            idx_to_keep = self._get_idx_to_keep_by_rms_window(
                self.mol.GetConformers(),
                rms_threshold,
                rms_only_heavy_atoms,
                idx_to_keep,
            )
            ...
            energies = np.array(
                [conformer.GetDoubleProp("energy") for conformer in conformers]
            )
        ...
        if not should_skip_step_execution:
            ...
            for conformer_index in range(len(self.mol.GetConformers())):
                ...
                if not os.path.isdir(dir_struct):
                    self.save_to_disk(
                        conformer_indices_to_output=[conformer_index],
                        output_folder=dir_struct,
                    )

                    _ = method.execute(
                        os.path.join(dir_struct, f"{conformer_basename}.xyz"),
                        self._dir_step,
                        self.charge,
                        self.multiplicity,
                        self._n_cores,
                        self._max_RAM_per_core_in_MB,
                    )
                ...
                self.mol.GetConformer(conformer_index).SetDoubleProp(
                    "energy", spp["energy_tot"] / kJdivmol_per_hartree
                )
    ...
    def sort_by_energy(self, step_name="sort_energy"):
        ...
        if not should_skip_step_execution:
            ...
            energies = np.array(
                [
                    conformer.GetDoubleProp("energy") * kJdivmol_per_hartree
                    for conformer in conformers
                ]
            )
    ...
    def filter_by_energy_window(self, energy_window_in_kJdivmol):
        ...
        def internal_filter(conformers):
            energies = np.array(
                [
                    conf.GetDoubleProp("energy") * kJdivmol_per_hartree
                    for conf in conformers
                ]
            )
            relative_energies = energies - energies.min()
            idx_to_keep = np.argwhere(
                relative_energies <= energy_window_in_kJdivmol
            ).flatten()
            return idx_to_keep
    ...
    def _get_idx_to_keep_by_rms_window(
        self, conformers, rms_threshold, rms_only_heavy_atoms=True, idx_to_keep=None
    ):
        energies = np.array(
            [conf.GetDoubleProp("energy") * kJdivmol_per_hartree for conf in conformers]
        )
        ...
        for idx0 in range(n_conf):
            for idx1 in range(idx0 + 1, n_conf):
                ...
                if idx_to_keep[idx0] and idx_to_keep[idx1]:
                    rms = ConformerGenerator.calculate_best_rms(
                        self.mol,
                        self.mol,
                        idx0,
                        idx1,
                        rms_only_heavy_atoms=rms_only_heavy_atoms,
                    )
    ...
    def filter_by_rms_window(self, rms_threshold, rms_only_heavy_atoms=True):
        ...
        def internal_filter(conformers):
            return self._get_idx_to_keep_by_rms_window(
                conformers, rms_threshold, rms_only_heavy_atoms=rms_only_heavy_atoms
            )

        self.filter_by_function(internal_filter, step_name="filter_by_rms_window")
        return self.current_step_number

    def filter_by_maximum_number_of_conformers_to_keep(
        self, maximum_number_of_conformers_to_keep, sort_conformers_by_energy=True
    ):
        ...
        self.filter_by_function(
            internal_filter, step_name="filter_by_maximum_number_of_conformers_to_keep"
        )
        return self.current_step_number
    ...
    def _log_results(self, last=False, title=None, should_skip_step_execution=False):
        ...
        if self.current_step_number == 0:
            ...
            if should_skip_step_execution:
                print(
                    f"- {self.step_names[index_to_log]}, n_confs: {len(self.mol.GetConformers())}, was loaded from previous calculation"
                )
            else:
                print(
                    f'- {self.step_names[index_to_log]}, n_confs: {len(self.mol.GetConformers())}, time_spent: {"{:.2f}".format(last_step_time)} s'
                )
    ...
    def save_to_disk(
        self,
        conformer_indices_to_output=None,
        output_folder=None,
        save_xyz_file=True,
        save_attached_file=False,
        save_last_log_file=False,
    ):
        ...
        for conformer_index in conformer_indices_to_output:
            ...
            conformer_basename = f"{self.name}_c{conformer_index:03d}"
            if save_xyz_file:
                ConformerGenerator.save_conformer_to_xyz_file(
                    conformer,
                    filepath_xyz=os.path.join(
                        output_folder, f"{conformer_basename}.xyz"
                    ),
                )
            ...
            if save_attached_file:
                if conformer.HasProp("attached_file"):
                    ...
                    shutil.copy(
                        attached_file,
                        os.path.join(
                            output_folder, f"{conformer_basename}{file_extension}"
                        ),
                    )
            ...
            if save_last_log_file:
                shutil.copy(
                    ...
                    os.path.join(output_folder, "log_output.dat"),
                )

    def copy_output(self, destination_folder, excluded_extensions=[], step_number=-1):
        shutil.copytree(
            os.path.join(self.dir_job, self.step_folders[step_number], "out"),
            destination_folder,
            ignore=shutil.ignore_patterns(*excluded_extensions),
...
class ORCA(ABC):
    ...
    def execute(
        ...
        try:
            ...
            if (
                e.args[0]
                == "The calculation did not converge or did not terminate normally."
            ):
    ...
    def _concatenate_output(self):
        ...
        with open(f"{self.structname}.orcacosmo", "w") as file:
            ...
            if self.mol:
                ...
                adjacency_matrix = Chem.rdmolops.GetAdjacencyMatrix(
                    self.mol, useBO=True
                )
                for i_row in range(adjacency_matrix.shape[0]):
                    line = (
                        "".join(
                            ["{:4d}".format(int(v)) for v in adjacency_matrix[i_row, :]]
                        )
                        + "\n"
                    )
                    file.write(line)
...
class ORCA_XTB2_ALPB(ORCA):
    def __init__(self, solvent):
        ...
        super().__init__(method_description="XTB2_ALPB", step_name="orca_xtb2_alpb")
...
class ORCA_DFT_FINAL(ORCA):
    def __init__(
        self,
        method_description="DFT_BP86_def2-TZVP+def2-TZVPD_SP",
        step_name="ORCA_DFT_final",
    ...
    def _write_input(self):
        ...
        lines.append(
            f"* xyzfile {self.charge} {self.multiplicity} {self.filename_final_xyz}"
        )
        lines.append("")
...
class ORCA_DFT_CPCM(ORCA):
    def __init__(
        self,
        method_description="DFT_CPCM",
        step_name="ORCA_DFT_CPCM",
        cpcm_radii=None,
        ...
        if cut_area_in_angstrom_squared is not None:
            self._cut_area_in_bohr_squared = cut_area_in_angstrom_squared / (
                angstrom_per_bohr * angstrom_per_bohr
            )
...
class ORCA_DFT_CPCM_FAST(ORCA_DFT_CPCM):
    def __init__(self, cpcm_radii=None, cut_area_in_angstrom_squared=None):
        ...
        super().__init__(
            method_description="DFT_CPCM_BP86_def2-TZVP(-f)",
    ...
    def _write_input(self):
        ...
        lines.append("")
        lines.append(
            f"! DFT {self.optimize_kw} CPCM BP86 def2-TZVP(-f){parallel_string}"
        )
...
class ORCA_DFT_CPCM_FINAL(ORCA_DFT_CPCM):
    def __init__(
        ...
        super().__init__(
            method_description="DFT_CPCM_BP86_def2-TZVP+def2-TZVPD_SP",
...
if __name__ == "__main__":
    ...
    with open(args.structures_file, "r") as f:
        ...
        for line in reader:
            if len(line) not in [4, 5]:
                raise ValueError(
                    f"Following line does not contain 4 values seperated by tabs [name, SMILES, xyz_file, charge]: {line}"
                )
            else:
                do_geometry_optimization = True
                if len(line) == 5:
                    name, smiles, xyz_file, charge, do_geometry_optimization = [
                        val.strip() for val in line
                    ]
    ...
    if len(cpcm_radii) > 0:
        ...
        if len(atomic_numbers_without_radii) > 0:
            raise RuntimeError(
                f"No CPCM radii were specified for the following atomic numbers: {sorted(atomic_numbers_without_radii)}"
            )

    for (name, (smiles, xyz_file, charge, do_geometry_optimization)), n_atoms in zip(
        name_smiles_dct.items(), number_of_atoms, strict=True
    ):

        with ConformerGenerator(
            name, continue_calculation=args.continue_calculation, n_cores=args.n_cores
        ) as cg:
            ...
            if not is_single_ion:
                # # gas
                n = cg.setup_initial_structure(
                    smiles, xyz_file, charge, title="gas calculation"
                )
                ...
                method = ORCA_DFT_FINAL(
                    do_geometry_optimization=do_geometry_optimization
                )
                cg.calculate_orca(method)
            ...
            method = ORCA_DFT_CPCM_FINAL(
                cg.mol,
                cpcm_radii=cpcm_radii,
                cut_area_in_angstrom_squared=0.01,
                do_geometry_optimization=do_geometry_optimization,
            )
            cg.calculate_orca(method)

            cg.save_to_disk(
                save_xyz_file=False, save_attached_file=True, save_last_log_file=True
            )
